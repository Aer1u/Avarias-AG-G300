                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-[#0A0F1A]">
                    <div className="p-6">
                      ...
                      <div className="w-full overflow-x-auto no-scrollbar">
                        <div className="min-w-[900px] border border-white/[0.05] rounded-[2rem] bg-white/[0.02] backdrop-blur-xl overflow-hidden shadow-2xl">
                          
                          {/* Cabeçalho da Tabela */}
                          <div className="grid grid-cols-[3rem_1fr_1fr_1.5fr_1.5fr_1.5fr_120px] gap-4 px-6 py-4 bg-white/[0.02] border-b border-white/[0.05] items-center">
                            ...
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest text-center">Sistema (A501/G501)</span>
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest text-center">Estornos</span>
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest text-center">Métricas</span>
                          ...
                          <div className="flex flex-col">
                            {lote.items.map((item, idx) => (
                              <motion.div 
                                ...
                                key={item.id} 
                                className={cn(
                                  "grid grid-cols-[3rem_1fr_1fr_1.5fr_1.5fr_1.5fr_120px] gap-4 px-6 py-4 items-center group/row hover:bg-white/[0.02] transition-colors border-b border-white/[0.02] last:border-0 relative"
                                )}
                              >
                                {/* 1. Índice */}
                                ...
                                <div className="flex items-center gap-2 pl-2">
                                  <Clock size={12} className="text-blue-500/70 shrink-0" />
                                ...
                                {/* 5. Estornos */}
                                <div className="flex items-center justify-center gap-3">
                                  <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/5 rounded-lg border border-amber-500/10">
                                    <span className="text-[8px] font-bold text-amber-500">A</span>
                                    <span className="text-[10px] font-black text-amber-200/70 font-mono">{item.estorno_a501 || "---"}</span>
                                  </div>
                                  <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/5 rounded-lg border border-amber-500/10">
                                    <span className="text-[8px] font-bold text-amber-500">G</span>
                                    <span className="text-[10px] font-black text-amber-200/70 font-mono">{item.estorno_g501 || "---"}</span>
                                  </div>
                                </div>
...
let parts = content.split(OLD_START);
if (parts.length === 2) {
  let endIdx = parts[1].indexOf('</AnimatePresence>') + '</AnimatePresence>'.length;
  let finalContent = parts[0] + NEW_CONTENT + parts[1].substring(endIdx);
  fs.writeFileSync(filepath, finalContent, 'utf-8');
  console.log("Success");
} else {
  console.log("Could not find OLD_START");
}
