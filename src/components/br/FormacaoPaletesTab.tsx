...
function formatDeposito(val: any, rawDep?: string | null) {
  if (rawDep && typeof rawDep === "string" && rawDep.trim()) {
    return rawDep.trim().toUpperCase()
  }
  if (typeof val === "string" && val.trim()) {
    const s = val.trim().toUpperCase()
    if (s.startsWith("Z") || s.startsWith("F")) return s
    return `Z${s}`
  }
  if (typeof val === "number") {
    if (val < 0) return `F${Math.abs(val)}`
    if (val === 0) return "Z2"
    return `Z${val}`
  }
  return "Z2"
...
export default function FormacaoPaletesTab({
  ...
  // Manual Typing / Import Tab states
  const [importTab, setImportTab] = useState<"colar" | "digitar">("digitar")
  const [manualPosicao, setManualPosicao] = useState("")
  const [manualCodigo, setManualCodigo] = useState("")
  const [manualDescricao, setManualDescricao] = useState("")
  const [manualDeposito, setManualDeposito] = useState("Z2")
  const [manualQtd, setManualQtd] = useState<number>(1)
  const [showCodeSuggestions, setShowCodeSuggestions] = useState(false)

  // Map for auto-filling description
  const descriptionMap = useMemo(() => {
    const map = new Map<string, string>()
    if (Array.isArray(baseCodigos)) {
      baseCodigos.forEach(item => {
        const cod = item["Código"] || item["codigo"]
        const desc = item["Descrição"] || item["descricao"]
        if (cod) {
          map.set(String(cod).trim().toUpperCase(), String(desc || ""))
        }
      })
    }
    return map
  ...
  const filteredCodeSuggestions = useMemo(() => {
    if (!manualCodigo.trim() || !baseCodigos) return []
    const term = manualCodigo.trim().toLowerCase()
    return baseCodigos.filter(b => {
      const c = String(b["Código"] || b["codigo"] || "").toLowerCase()
      const d = String(b["Descrição"] || b["descricao"] || "").toLowerCase()
      return c.includes(term) || d.includes(term)
    }).slice(0, 8)
  }, [manualCodigo, baseCodigos])

  const handleSelectCode = (item: any) => {
    const cod = String(item["Código"] || item["codigo"] || "").trim().toUpperCase()
    const desc = String(item["Descrição"] || item["descricao"] || "").trim()
    setManualCodigo(cod)
    setManualDescricao(desc)
    setShowCodeSuggestions(false)
  }

  const handleCodigoTyped = (val: string) => {
    setManualCodigo(val)
    setShowCodeSuggestions(true)
    const norm = val.trim().toUpperCase()
    if (descriptionMap.has(norm)) {
      setManualDescricao(descriptionMap.get(norm) || "")
    } else {
      const match = baseCodigos?.find(b => String(b["Código"] || b["codigo"] || "").trim().toUpperCase() === norm)
      if (match) {
        setManualDescricao(String(match["Descrição"] || match["descricao"] || "").trim())
      }
    }
  }

  const handleAddManualItem = () => {
    if (!manualPosicao.trim()) {
      alert("Por favor, informe a Posição.")
      return
    }
    if (!manualCodigo.trim()) {
      alert("Por favor, informe o Código/Produto.")
      return
    }
    if (!manualQtd || manualQtd <= 0) {
      alert("Por favor, informe uma Quantidade válida (maior que 0).")
      return
    }

    const pos = manualPosicao.trim().toUpperCase()
    const cod = manualCodigo.trim().toUpperCase()
    const desc = manualDescricao.trim() || descriptionMap.get(cod) || "-"
    const dep = (manualDeposito.trim() || "Z2").toUpperCase()
    const qty = manualQtd

    const line = `${pos}\t${cod}\t${desc}\t${dep}\t${qty}`
    setImportText(prev => prev ? `${prev}\n${line}` : line)

    // Clear single item inputs, keep position and deposit for convenience
    setManualCodigo("")
    setManualDescricao("")
    setManualQtd(1)
  }
                        ...
                        {selectedPosGroup.items.map((item: PosAvaria) => (
                          <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20">
                            ...
                            <td className="px-4 py-3.5 text-center">
                              {(() => {
                                const depStr = formatDeposito(item.estoque, item.documento)
                                const isF2 = depStr.toUpperCase().startsWith("F")
                                return (
                                  <span
                                    className={cn(
        ...
        {showImportModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              {/* Tab Selector: Digitar vs Colar */}
              <div className="flex items-center gap-2 p-1 bg-slate-100 dark:bg-slate-900 rounded-xl mb-4 border border-slate-200/50 dark:border-slate-800">
                <button
                  type="button"
                  onClick={() => setImportTab("digitar")}
                  className={cn(
                    "flex-1 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-2",
                    importTab === "digitar"
                ...
                >
                  📋 Colar da Planilha (Excel / Sheets)
                </button>
              </div>
                ...
                {importTab === "digitar" ? (
                  <div className="space-y-4 bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-200/60 dark:border-slate-800/80">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={manualPosicao}
                          onChange={e => setManualPosicao(e.target.value.toUpperCase())}
                          placeholder="Ex: AVA00040"
                          className="w-full h-10 px-3 text-xs font-mono font-bold rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                        />
                      </div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          type="text"
                          value={manualCodigo}
                          onChange={e => handleCodigoTyped(e.target.value)}
                          onFocus={() => setShowCodeSuggestions(true)}
                          placeholder="Ex: 3798-01"
                          className="w-full h-10 px-3 text-xs font-mono font-bold rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                        />
                        {/* Suggestions dropdown */}
                        {showCodeSuggestions && filteredCodeSuggestions.length > 0 && (
                          <div className="absolute left-0 right-0 top-full mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 overflow-hidden max-h-48 overflow-y-auto custom-scrollbar">
                            {filteredCodeSuggestions.map((item, idx) => (
                              <button
                                key={idx}
                                type="button"
                                onClick={() => handleSelectCode(item)}
                                className="w-full text-left px-3 py-2 text-xs hover:bg-emerald-50 dark:hover:bg-slate-800 transition flex flex-col cursor-pointer border-b border-slate-100 dark:border-slate-800/40 last:border-0"
                              >
                                <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400">{item["Código"] || item["codigo"]}</span>
                                <span className="text-[10px] text-slate-500 dark:text-slate-400 truncate">{item["Descrição"] || item["descricao"]}</span>
                              </button>
                            ))}
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          min={1}
                          value={manualQtd}
                          onChange={e => setManualQtd(parseInt(e.target.value, 10) || 0)}
                          placeholder="Qtd"
                    ...
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold text-xs uppercase tracking-wider transition active:scale-[0.99] flex items-center justify-center gap-2 cursor-pointer shadow-sm"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full h-48 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-4 text-xs font-mono text-slate-800 dark:text-slate-300 focus:outline-none focus:border-emerald-500 resize-none overflow-y-auto custom-scrollbar"
                      placeholder="Exemplo:&#10;AVA00002	9751-02	VENT 40CM VTX-40P-8P 220V/60Hz PAREDE	Z2	2&#10;AVA00002	2026-02	VENT50CM VTX-50-8P 220V/60Hz SUPER TURBO	F2	30"
                    />
                  </div>
                )}
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40">
                        {importText.trim().split('\n').filter(l => l.trim()).slice(0, 15).map((line, idx) => {
                          ...
                          const cod = cols[1] || '-'
                          const desc = cols[2] || '-'
                          const dep = (cols[3] || 'Z2').toUpperCase()
                          const qty = cols[4] || '0'
                          const isF = dep.startsWith('F')
                          return (
                            <tr key={idx} className="text-xs">
                              <td className="px-2 py-1 font-mono font-bold text-slate-700 dark:text-slate-300">{pos}</td>
                              <td className="px-2 py-1 font-mono font-bold text-slate-600 dark:text-slate-400">{cod}</td>
                              <td className="px-2 py-1 text-[11px] text-slate-600 dark:text-slate-400 truncate max-w-[180px]">{desc}</td>
                              ...
                              <td className="px-2 py-1 text-right font-bold text-slate-800 dark:text-slate-200">{qty}</td>
                            </tr>
                          )
                        })}
                      </tbody>
