...
export default function BRModulePage() {
  ...
  // Load base_codigos Reference Data
  const [baseCodigos, setBaseCodigos] = useState<any[]>([])
  const [legacyCount, setLegacyCount] = useState(0)
  const [isMigrating, setIsMigrating] = useState(false)

  const fetchBaseCodigos = async () => {
    try {
      ...
      console.error("Erro ao buscar base de codigos:", err.message)
    }
  }
  ...
  // Memoized description map for O(1) lookups
  const descriptionMap = React.useMemo(() => {
    const map = new Map<string, string>()
    baseCodigos.forEach(item => {
      if (item["Código"]) {
        map.set(String(item["Código"]).trim().toUpperCase(), item["Descrição"] || "")
      }
    })
    return map
  }, [baseCodigos])
  ...
  // Load controle_avarias (Formed Pallets) mapped from new tables
  const fetchControleAvarias = async (period: "hoje" | "semanal" | "mensal" = "hoje") => {
    setLoadingControle(true)
    try {
      // ── Server-side date filter to avoid Supabase 1000-row limit ──────────
      const now = new Date()
      ...
      // For "hoje": exact match on criacao OR entrega. For semanal/mensal: >= start date.
      const orFilter = period === "hoje"
        ? `criacao.eq.${today},entrega.eq.${today}`
        : `criacao.gte.${startDate},entrega.gte.${startDate}`
      ...
      const mapped = (docs || []).map((doc: any) => {
        ...
        const totalQtd = docItems.reduce((acc: number, item: any) => acc + (item.quantidade || 0), 0)

        const exclusions = docItems.find(item => item.itens_excluido)?.itens_excluido || []
        const parsedExclusions = Array.isArray(exclusions) ? exclusions : []

        const resultItems: any[] = []
        docItems.forEach((active: any) => {
          const excl = parsedExclusions.find((e: any) => e.codigo === active.codigo)
          const codeUpper = String(active.codigo).trim().toUpperCase()
          const desc = descriptionMap.get(codeUpper) || "—"
          resultItems.push({
            codigo: active.codigo,
            descricao: desc,
            qtdOriginal: excl ? (excl.qtdOriginal || active.quantidade) : active.quantidade,
            qtdFinal: active.quantidade,
            incluido: true
          })
        })

        parsedExclusions.forEach((excl: any) => {
          if (excl.excluido) {
            const codeUpper = String(excl.codigo).trim().toUpperCase()
            const desc = descriptionMap.get(codeUpper) || "—"
            resultItems.push({
              codigo: excl.codigo,
              descricao: desc,
              qtdOriginal: excl.qtdOriginal || 0,
              qtdFinal: 0,
              incluido: false
            })
          }
        ...
        return {
          id: doc.id,
          posicao: doc.position,
          reserva: doc.reserva,
          remessa: doc.remessa,
          documento: doc.documento ? String(doc.documento) : "",
          type: doc.type,
          criacao: doc.criacao,
          entrega: doc.entrega || null,
          status: doc.status,
          quantidade: totalQtd,
          itens_json: JSON.stringify(resultItems),
          codigos: docItems.map((item: any) => item.codigo).join(", ")
        }
      })

      setControleRaw(mapped)
    } catch (err: any) {
  ...
  const handleMigrateData = async () => {
    ...
    try {
      ...
      if (!oldData || oldData.length === 0) {
        alert("Nenhum dado legado encontrado na tabela controle_avarias.")
        setLegacyCount(0)
        return
      }
      ...
      for (const row of oldData) {
        ...
        if (insDocErr) {
          console.error(`Erro ao migrar doc para remessa ${row.remessa}:`, insDocErr)
          continue
        }
        ...
        try {
          if (row.itens_json) {
            items = JSON.parse(row.itens_json)
          }
        } catch (e) {
          console.error(e)
        }
        ...
        if (items.length > 0) {
          const exclusions = items
            .filter(item => !item.incluido || item.qtdFinal !== item.qtdOriginal)
            .map(item => ({
              codigo: item.codigo,
              qtdOriginal: item.qtdOriginal,
              qtdFinal: item.qtdFinal,
              excluido: !item.incluido
            }))

          const activeItems = items.filter(item => item.incluido && item.qtdFinal > 0)
          if (activeItems.length > 0) {
            const relPayload = activeItems.map(item => ({
              remessa: row.remessa,
              codigo: item.codigo,
              quantidade: item.qtdFinal,
              itens_excluido: exclusions.length > 0 ? exclusions : null
            }))
            ...
            if (insRelErr) {
              console.error(`Erro ao inserir itens da remessa ${row.remessa}:`, insRelErr)
            }
          }
        }
        migratedCount++
      ...
      alert(`Migração concluída! ${migratedCount} novos registros migrados.`);
      checkLegacyData()
      fetchControleAvarias()
    } catch (err: any) {
      console.error(err)
  ...
  const handleImportRelacao = async () => {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        const rawDeposito = String(cols[3] || 'Z2').trim().toUpperCase()
        const isFDep = rawDeposito.startsWith('F')
        const numDep = parseInt(rawDeposito.replace(/[^\d]/g, ''), 10) || 2
        const cleanEstoque = isFDep ? -Math.abs(numDep) : numDep
        const depDocStr = rawDeposito || (isFDep ? `F${numDep}` : `Z${numDep}`)
  ...
  // Formar Palete submission (Two-table structure)
  const handleConfirmarFormarPalete = async () => {
    ...
    try {
      const remessaInt = parseInt(remessaNum.replace(/[^\d]/g, ''), 10)
      const docInt = parseInt(documentoSM.replace(/[^\d]/g, ''), 10)
      const reservaInt = reservaNum.trim() ? parseInt(reservaNum.replace(/[^\d]/g, ''), 10) : null
      ...
      const exclusions = editableItems
        .filter(item => !item.incluido || item.quantidade !== item.qtdOriginal)
        .map(item => ({
          codigo: item.codigo,
          qtdOriginal: item.qtdOriginal,
      ...
      // 1. Insert into documento_palete
      const docPayload = {
        position: selectedPosGroup.posicao,
        remessa: remessaInt,
        documento: docInt,
        reserva: reservaInt,
      ...
      // 2. Insert into relacao_documento
      const activeItemsToSave = activeItems.filter(item => item.quantidade > 0)
      if (activeItemsToSave.length > 0) {
        const relPayload = activeItemsToSave.map(item => ({
          remessa: remessaInt,
          codigo: item.codigo,
          quantidade: item.quantidade,
          itens_excluido: exclusions.length > 0 ? exclusions : null
  ...
  const handleDeleteControle = async (id: number) => {
    ...
    try {
      const doc = controleRaw.find(c => c.id === id)
      if (!doc) throw new Error("Registro não encontrado localmente.")

      // 1. Delete from relacao_documento first
      const { error: relErr } = await supabase
        .from('relacao_documento')
        .delete()
        .eq('remessa', doc.remessa)
      if (relErr) throw relErr

      // 2. Delete from documento_palete
  ...
  // Print pallet record
  const handlePrintControle = (row: any, showQrCode: boolean = false, autoPrint: boolean = true) => {
    const now = new Date()
    ...
    const docSap = row.documento ? String(row.documento).trim() : ''
    const qrUrl = (showQrCode && docSap) ? `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(docSap)}` : ''

    const html = `<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Palete ${row.posicao} - Doc SAP ${docSap || '-'}</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;padding:32px;color:#1e293b}
@media print{.no-print{display:none!important}}
.no-print-bar{margin-bottom:20px;display:flex;justify-content:space-between;align-items:center;background:#f8fafc;border:1px solid #e2e8f0;padding:12px 18px;border-radius:12px}
.no-print-btn{background:#10b981;color:#fff;border:none;padding:8px 16px;border-radius:8px;font-weight:800;font-size:12px;cursor:pointer}
.hdr{border-bottom:3px solid #10b981;padding-bottom:16px;margin-bottom:24px;display:flex;justify-content:space-between;align-items:center}
...
.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:24px;align-items:stretch}
.card{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px 16px;display:flex;flex-direction:column;justify-content:center}
...
.ftr{margin-top:28px;padding-top:14px;border-top:1px solid #e2e8f0;font-size:10px;color:#94a3b8;display:flex;justify-content:space-between}
</style>
</head><body>
${!autoPrint ? `<div class="no-print no-print-bar">
  <div>
    <strong style="font-size:13px;color:#0f172a">Ficha do Palete ${row.posicao} (QR Code SAP)</strong>
    <p style="font-size:11px;color:#64748b">Documento SAP: ${docSap || 'Não informado'}</p>
  </div>
  <button class="no-print-btn" onclick="window.print()">🖨️ Imprimir Ficha</button>
</div>` : ''}
<div class="hdr">
  <div>
    <div class="pos"><span class="pos-pre">${row.posicao.slice(0,-2)}</span><span class="pos-suf">${row.posicao.slice(-2)}</span></div>
    <div class="sub">Portal BR · G300 · Gestão de Avarias</div>
  </div>
</div>
<div class="grid">
  ...
  <div style="display:flex;flex-direction:column;gap:14px">
    <div class="card" style="align-items:center;text-align:center">
      <div class="lbl">Documento SAP</div>
      <div class="val" style="color:#059669;margin-bottom:4px">${docSap || '-'}</div>
      ${qrUrl ? `<img src="${qrUrl}" alt="QR Code SAP" style="width:80px;height:80px;border-radius:6px;margin-top:4px" />` : ''}
    </div>
  </div>
</div>
...
${itensHtml}
<div class="ftr">
  <span>Portal BR · MKBR e G300 · Gestão de Avarias</span>
