...
def get_movement_data(period: str = "hoje", xl=None):
    try:
        ...
        if period == "hoje":
            ...
            # Inicia na segunda-feira da semana atual
            monday = (today - pd.Timedelta(days=today.weekday())).normalize()
            df_filtered = df[df['dt'] >= monday]
        elif period == "mensal":
            df_filtered = df[df['dt'] >= (today - pd.Timedelta(days=365))]
        else: # recente
            df_filtered = df
        ...
        for _, row in df_latest.iterrows():
            ...
            result.append({
                "data": dt_obj.strftime('%d/%m/%Y') if pd.notna(row.get('dt')) else "-",
...
def get_clean_data(xl=None, unalloc_xl=None):
    ...
    if not df_reg.empty:
        ...
        # 3. Tratar posições bloqueadas (Fechado/Bloqueado)
        if 'status' in df.columns:
            df['status'] = df['status'].fillna('Aberto').astype(str).str.strip()
            # Se a posição estiver Fechada ou Bloqueada, ela não deve contabilizar para ocupação ou estoque nos totais
            is_blocked = df['status'].str.lower().isin(['fechado', 'bloqueado'])
            df.loc[is_blocked, 'quantidade_total'] = 0
            df.loc[is_blocked, 'paletes'] = 0
            df.loc[is_blocked, 'capacidade'] = 0
            df['is_blocked'] = is_blocked
        else:
            df['is_blocked'] = False            
        # 4. Adicionar os não alocados (S/P) ao final
    ...
    for rc in required_cols:
        if rc not in df.columns:
            if rc == 'is_blocked':
                df[rc] = False
            elif rc in ['quantidade_total', 'paletes', 'capacidade', 'qtd_tombada', 'qtd_molhado', 'qtd_por_palete']:
                df[rc] = 0
            elif rc == 'posicao':
                df[rc] = 'S/P'
            elif rc == 'status':
    ...
    # Garantir que is_blocked seja booleano (especialmente aps concats)
    df['is_blocked'] = df.get('is_blocked', False).fillna(False).astype(bool)
...
async def get_stats(period: str = "hoje"):
    try:
        ...
        # EXCLUIR POSIÇÕES BLOQUEADAS DAS CONTAGENS DE DRIVE E OCUPAÇÃO
        stats_df = df[~df['is_blocked']] if 'is_blocked' in df.columns else df
        
        return {
            "total_pallets": int(df['paletes'].sum()),
            "total_quantity": int(df['quantidade_total'].sum()),
            "total_positions": int(stats_df[stats_df['posicao'] != 'S/P']['posicao'].nunique()),
            "total_skus": qty_totals.get("total_skus", 0),
            "avg_occupancy": float(stats_df[stats_df['capacidade'] > 0]['ocupacao'].mean()) if not stats_df[stats_df['capacidade'] > 0].empty else 0,
